...
from .circular_cartpole_robot_cfg import (
    CIRCULAR_CARTPOLE_ROBOT_L3_CFG,
    upward_pos,
    joint_effort_scale,
)
...
flex_2_pole_index = 3
flex_3_pole_target_pos = upward_pos
flex_3_pole_index = 4
...
class CircularCartpoleSceneCfg(InteractiveSceneCfg):
    ...
    # robot
    robot: ArticulationCfg = CIRCULAR_CARTPOLE_ROBOT_L3_CFG.replace(
...
class EventCfg:
    ...
    reset_flex_3_pole_position = EventTerm(
        func=mdp.reset_joints_by_offset,
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=["flex_2_to_flex_3"]),
            "position_range": (-0.25 * math.pi, 0.25 * math.pi),
            "velocity_range": (-0.25 * math.pi, 0.25 * math.pi),
        },
    )
...
class RewardsCfg:
    ...
    alignment_flex_3_reward = RewTerm(
        func=mdp.rk_pole_rotation_reward_multi,
        weight=0.6,
        params={
            "linear_weight": linear_weight,
            "exp_weight": exp_weight,
            "pole_index_list": [flex_3_pole_index],
            "target_pos_list": [flex_3_pole_target_pos],
        },
    )

    action_magnitude = RewTerm(func=mdp.rk_action_l2, weight=-0.01)
    action_smoothness = RewTerm(func=mdp.rk_action_rate_l2, weight=-0.01)
